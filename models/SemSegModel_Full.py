...
from transformer import VisMaskTransformer

class SemSegModelFull(nn.Module):
    def __init__(self,
    ...
    def load_pretrained(self, pretrained_path):
        ...
        print(f'[SemSegModelFull] loaded: missing={len(msg.missing_keys)}, '
              f'unexpected={len(msg.unexpected_keys)}')

    def _encode_pos(self, xyzts_flat):
        xyz = xyzts_flat[..., :3]
        ...
        if t.numel() > 0:
            ...
            if t_max > 0:
                t = t / t_max
        return self.encoder_pos_linear(torch.cat([xyz, t], dim=-1))
    ...
    def forward(self, clips):
        ...
        L, N_s, C = features.shape[1], features.shape[2], features.shape[3]

        xyzts = []
        for t_idx, xyz in enumerate(torch.split(xyzs, 1, dim=1)):
            xyz = xyz.squeeze(1)
        ...
        pos_emb = self._encode_pos(xyzts_flat)

        x = feats_flat + pos_emb
        x_empty = torch.zeros(B, 0, C, device=clips.device)
        x, _ = self.encoder_transformer(x, x_empty)
