    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                neighs = neighbor_xyz_grouped.permute(0, 2, 3, 1)
                descriptor = getDescribe(anchor_xyz, neighs,
                                        SHOTradius=self.r,
                                        useInterpolation=True, useNormalization=False,
                                        nnidx=idx
                )
                new_shot_descriptor.append(descriptor.detach())
            ...
            new_xyz_neighbor = torch.stack(tensors=new_xyz_neighbor, dim=-1)
            new_xyzs_neighbors.append(new_xyz_neighbor)

            new_shot_descriptor = torch.stack(tensors=new_shot_descriptor, dim=2)
        ...
        new_xyzs_neighbors = torch.stack(tensors=new_xyzs_neighbors, dim=1)
        new_xyzs_neighbors = new_xyzs_neighbors.permute(0, 1, 3, 5, 4, 2)

        new_shot_descriptors = torch.stack(tensors=new_shot_descriptors, dim=1)

        return new_xyzs, new_features, new_xyzs_neighbors, new_shot_descriptors
